...
public class FileSummay
{
	 ...
	 * @param file
	 */
	public FileSummay(String innerPath, IFile file)
	{
	...
	public IdDef findIDDef(IdRef ref)
	{
		...
		{
			IdDef id = list.get(i);
			if (id.getID().equals(ref.getText()))
			{
				return id;
			}
		}
		...
		for (int i = dependRefs.size() - 1; i >= 0; i--)
		{
			...
			if (depend.filePath != null && !depend.filePath.isEmpty())
			{
				IFile dependFile = Project.getSourceFile(file.getProject(), depend.filePath);
				if (dependFile.exists())
				{
					FileSummay data = Project.getFileSummay(dependFile);
					if (data != null)
					{
		...
		for (ParseTree child : root.children)
		{
			...
			if (tagName.equals("bitmap"))
			{
				...
				for (AttributeContext attribute : attributes)
				{
					...
					{
						idBitmaRef = new IdRef(this, IdRef.BMP, txt, start, stop);
		...
		for (ParseTree child : root.children)
		{
			...
			if (tagName.equals("bitmapReader"))
			{
				...
				for (AttributeContext attribute : attributes)
				{
					...
					{
						bitmapRef = new IdRef(this, IdRef.BMP, txt, start, stop);
					}
				}
		...
		for (ParseTree child : root.children)
		{
			...
			if (tagName.equals("format"))
			{
				...
				for (AttributeContext attribute : attributes)
				{
					...
					{
						idRefs.add(new IdRef(this, IdRef.FILTER, txt, start, stop));
					}
				}
			}
		}
	...
	private static int getType(String tagName, String attributeName)
	{
		if (typeTable == null)
		{
			...
			typeTable.put("ref.ref", IdRef.CONTROL);

			typeTable.put("image.bmp", IdRef.BMP);
			typeTable.put("filmAnimEffect.bmp", IdRef.BMP);
			typeTable.put("progressBar.bmp", IdRef.BMP);
			typeTable.put("countImage.bmp", IdRef.BMP);

			typeTable.put("text.format", IdRef.FORMAT);

			typeTable.put("label.format", IdRef.FORMAT);
			typeTable.put("label.text", IdRef.TXT);

			typeTable.put("labelButton.label", IdRef.TXT);
			typeTable.put("labelToggleButton.label", IdRef.TXT);
